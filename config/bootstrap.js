...
module.exports.bootstrap = async function() {
  ...
  var project = await Project.create({ title: 'Test Project', client: client.id, position: 1 }).fetch();
  var stage1 = await Stage.create({ title: 'Stage 1', project: project.id, position: 1 }).fetch();
  var stage2 = await Stage.create({ title: 'Stage 2', project: project.id, position: 2 }).fetch();
  ...
  var task2 = await Task.create({ title: 'Task 2', stage: stage1.id, position: 2, status: 'Not Started', due: moment().add(2, 'months').format('X') }).fetch();
  var task3 = await Task.create({ title: 'Task 3', stage: stage2.id, position: 3, status: 'Ready for Review', due: moment().add(2, 'months').format('X') }).fetch();
  var task4 = await Task.create({ title: 'Task 4', stage: stage2.id, position: 4, status: 'Not Started', due: moment().add(2, 'months').format('X') }).fetch();
